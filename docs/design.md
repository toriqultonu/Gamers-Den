...
Product: point-of-sale and floor management for a console gaming cafe (2× PS5, 2× PS4, growing), plus a tournament module.
...
Scope: single venue, one counter terminal (cafe PC), cloud sync for off-site owner access. 1 engineer / 1 month — every screen below exists in the prototype; nothing here is new invention.
...
| S2 | Overview | Occupancy, revenue, avg ticket, net profit; horizontally scrolling live-station cards (click → Floor); 30-day + day-of-week trends; stock watchlist; staff & shift closes; collapsible alerts rail (bell + unread badge) | Post-login landing (Admin) | Admin |
| S3 | Floor | Station cards with live countdowns — session blocks or tournament match time; session panel: start (no clock), ±30-min blocks, start/pause/resume clock, bill, end (blocked while balance due) | Sidebar; Overview card | All |
| S4 | Point of sale | Menu grid incl. Tournament category + bill panel; station bill or counter sale (toggle); member attach + points redemption; tournament player-name field; split payment; 80mm ticket preview | Sidebar (counter mode); Floor "Bill & take payment" / "Add food & drinks" (station mode) | All |
...
| S7 | Shift close | X-report by method; tournament-entries reconciliation strip; editable drawer count with live discrepancy; petty-cash list; Z print closes shift → login | Sidebar | All |
...
| S10 | Setup / Menu & stock | Admin: stations, pricing, staff (add cashier/manager with PIN, remove unless on shift), menu. Manager: menu & stock only | Sidebar | Admin, Manager |
| S11 | Print preview | Exact character-grid render of any artifact; reprint with reason | POS settle, shift close, job history | All |
| S12 | Tournaments | Live & upcoming / History tabs; bracket with per-match countdowns; "Now on «console»" live tiles; match board (start match → auto console assignment, +5 min); Manager rail: arrange/cancel, station blocks, finance analytics. Cashier rail: read-only guidance + POS shortcut | Sidebar (badge LIVE) | All (writes Manager+) |
...
| S3 | "No stations — add one in Setup" | Controls disabled + banner | Reserved stations refuse session start |
...
| S13 | n/a | Image-load failure keeps previous background | n/a |
...
|-----------|----------|--------|-----------|
| Button | primary, secondary, ghost, icon, block | default, hover, active, focus-visible, disabled, loading | `variant, size, loading, disabled` — labels centered |
| Tag | accent, neutral, outline | static | `variant` |
| StationCard | active, open, paused, locked, free, reserved (tournament), maintenance | selected outline, hover | `station, selected, onSelect` |
| SessionPanel | station, reserved, empty | running, paused, open, locked, match | `station` |
| CountdownClock | panel (50px), card (52px), match tile (26px) | running, paused, overtime, none | `remainingSec, state` |
| MenuItemCard | item, tournament entry | hover, low-stock, out-of-stock/full (disabled) | `item, onAdd` |
| BillPanel | station, counter | with/without member, tournament lines | `mode, sessionId?` |
| CartLine | — | qty=min removes | `line, onInc, onDec` |
...
| ReceiptPreview | receipt, z/x-report, tournament stub | rendering, ready, failed | `printJobId` |
| BracketView | 4/8/16/32 players | per-match: pending, live (console + countdown tag), decided | `rounds, canDecide` |
| MatchBox / PlayerRow | — | Winner ✓ chip (decidable), red W (winner), dimmed loser | `match, onWin` |
| LiveMatchTile | — | counting, TIME UP (accent fill) | `match` |
| MatchBoard row | pending, in-play | Start disabled when no free console / console busy; +5 min | `match, onStart, onExtend` |
| ChampionBanner | — | — | `winner, prize` |
| FinancePanel (Manager+) | — | positive/negative extra margin | `tournament` |
| DataTable, FieldInput, SegmentedChoice, ChipSelect, Dialog, SidebarNav, StatTile, BarChart, ProgressBar, AlertCard, AlertsRail (bell/expanded), SyncChip, TopBar | as before | — | — |
| AvatarSwatch | 6 colors | selected outline | `color, onPick` |
| ImagePicker | empty, preview | — | `value, onPick, onClear` |
...
| `color.accent` | user-selectable: Den Red `#ec3013` (default), Blue `#0f62fe`, Green `#198038` | same | Primary action, live marks, reserved borders |
...
| `color.paper` | `#ffffff` fixed | fixed | Receipt preview only |
...
Archivo throughout; headings weight 800, tight tracking. Base size scales with the Settings text-size choice: Compact 13px / Default 14px / Large 16px (everything sized in em-relative steps follows).
...
| `type.mono` | 11/1.6 ui-monospace | Receipt preview only |
...
| 1024–1279 | POS ticket column collapses behind a Preview button; Overview alerts rail starts collapsed |
...
Staffed counter, USB thermal ESC/POS ("POS Receipt Printer" — **assumed 80mm/203dpi; 58mm is a config switch**, see §7). Monochrome character grid (48 cols Font A), independent of screen theme by construction. Full cut + 4 feeds per artifact.

### P1 — Sale ticket
Header (GAMER'S DEN double-size, address/phone) · meta rows (TXN, STATION or TYPE "Counter sale", IN, OUT, CASHIER) · lines (`GAMING n×30M`, F&B `NAME ×qty`, `POINTS nPTS` negative) · TOTAL double-height + one row per tender (bKash/Nagad append last 6 of TrxID) · loyalty line (points earned · balance) · Code 128 barcode (below) · reprint band when reprinted: inverted `*** REPRINT (reason) · original date ***` under header.

### P2 / P3 — Z / X report
Z: shift id, opened/closed, operator, float, takings method × (gaming, F&B, tournament, total), expected vs counted vs discrepancy (double-height), expenses, counts, signature line, barcode = shift id. X: same minus drawer/discrepancy/signature, headed "X REPORT — INTERIM", no barcode.

### P4 — Expense voucher
Date-time, description, category, amount double-height, recorded-by, signature line.

### P5 — Tournament entry stub
Appended to P1 when entries are on the bill (or standalone reprint): inverted "TOURNAMENT ENTRY" line · tournament name(s) · player name (attached member or typed name; else "Walk-in guest") · **TOKEN #NN** double-height — the player's bracket seed, one token per entry · QR code · "Show this ticket at the bracket desk".

### Barcode / QR
- Code 128 subset B (transaction / shift ids): module ≥0.33mm, height 12mm, quiet zone ≥10 modules, native `GS k`.
...
| Profile | Avatar color | 6 swatches + reset (photo upload deferred — no camera at the counter) | Per staff login |

Terminal settings persist server-side per terminal; profile prefs per staff row.

---

## 7. Iconography & imagery

Lucide only (bell, printer, plus/minus, pause/play, search, log-out, alert-triangle, wifi-off, trophy). No emoji. The only photograph is the optional login background, shown under a dark overlay.
...
4. Owner phone access (<768px) deferred; desktop Overview covers MVP.
