# Gamer's Den — Execution Task List (1-Month Build)

Granular task list to build the whole app with Claude Code. **1 TASK = 1 Claude Code session = 1 prompt.**
Tasks are ordered by dependency — backend `B01…B20` first, frontend `F01…F16` after (FE can start at F01 once B01–B04 exist, since types generate from the backend OpenAPI). Each task ends with a green build, so errors never accumulate.

- **Source of truth:** `docs/design.md` (screens/states/tokens/prints) → `docs/api-contract.md` (**wins all conflicts** on shapes) → `docs/tournaments.md` (tournament rules/DDL) → `backend/ARCHITECTURE.md` + `frontend/ARCHITECTURE.md` (canonical names, invariants) → this file (execution order + prompts).
- **Scope:** 36 tasks ≈ 1 engineer × 4–5 weeks. Heavy tasks marked 🔴 (budget 1.5–2 sessions/days).
...
1. **Authority order:** `api-contract.md` wins on shapes; `design.md` wins on UI/states/prints; `tournaments.md` wins on bracket rules. Never invent an endpoint, field, error code, table, or token — canonical names live in the two `ARCHITECTURE.md` files (§4 each).
...
5. **Idempotency + one-TX settle are sacred:** money/print mutations take `Idempotency-Key`; settle writes everything (payment, blocks, stock, ledgers, entries, print job) in one DB transaction.
6. **Server time only:** all countdowns derive from server timestamps / `remainingSeconds`; clients never trust the local clock.
...
**Description:** `V001__baseline.sql` with the **verbatim** core DDL from `docs/backend-architecture.md` §2 (staff, terminal_settings, stations, pricing, members, shifts, sessions, session_blocks, items, stock_movements, carts, cart_lines, transactions, payment_splits, points_ledger, wallet_ledger, expenses, print_jobs, alerts, idempotency_keys, sync_outbox) + seed rows (Admin staff with BCrypt PIN, PS5/PS4 pricing, item categories). JPA entities for all tables, `validate` mode.
**Prompt:**
```
...
**Description:** `Idempotency-Key` handling per `docs/api-contract.md` §1: applies to `POST /payments`, `/print-jobs`, `/sessions/*/blocks`, `/wallet/*`, `/tournaments/*/entries`. Store key + request hash + response 48 h; identical retry replays stored response with `Idempotency-Replayed: true`; same key different body → 409 `IDEMPOTENCY_REPLAY`; missing key on a guarded route → 400.
**Prompt:**
```
...
**Description:** Stations CRUD (Admin writes): 409 `DUPLICATE_NAME`, delete → 409 `STATION_IN_USE`; `GET /stations` includes live session/match summary (stub the match part until B12). `GET/PUT /pricing` per consoleType with morning-discount fields; **new blocks only** — running sessions keep purchased prices (snapshot lives on `session_blocks.price`).
**Prompt:**
```
...
**Description:** `OPEN → RUNNING ⇄ PAUSED → LOCKED → CLOSED` with server-side block math. `POST /sessions` (409 `STATION_BUSY`; `STATION_RESERVED` hook stubbed until B12), `POST /sessions/{id}/blocks` (`{delta:±1}`, idempotent, −1 below paid/consumed → 409 `BLOCKS_CONSUMED`), `POST /sessions/{id}/clock` (`START|PAUSE|RESUME`, 409 `NO_BLOCKS`), `POST /sessions/{id}/end` (409 `SESSION_HAS_BALANCE`), reads incl. `remainingSeconds` computed from `consumed_sec`/`running_since`. Morning-rate snapshot per block. One live session per station (partial unique index already in DDL).
**Prompt:**
```
Read backend/ARCHITECTURE.md §5.1 and docs/api-contract.md (Sessions).
Implement TASK B06 from TASKLIST.md: session state machine + blocks + clock.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND unit tests cover every state transition + illegal ones, block math across pause/resume, morning-window boundary pricing, and Testcontainers proves the one-live-session index.
Show me the build output before saying you're finished.
```

### B07 — Catalog: items, stock, carts
`branch: task/B07-catalog-carts`
**Description:** Items CRUD (Manager+ writes; stock edits audit `stock_movements`), `POST /carts` (`{type: COUNTER}` or session-bound), `PUT /carts/{id}/lines` (`{itemId, qty}`, 0 removes, 409 `OUT_OF_STOCK`, unit-price snapshot on line).
**Prompt:**
```
...
**Description:** Member search/create (409 `DUPLICATE_PHONE`), detail + recent visits, `wallet/topup` (idempotent), `wallet/redeem-points` (1pt=৳1 → wallet, 409 `INSUFFICIENT_POINTS`). Ledgers are the source of truth; `members.wallet/points` kept consistent in the same TX. Earn/redeem-at-settle rules land in B10.
**Prompt:**
```
...
**Description:** `GET /sessions/{id}/bill`: gaming = **unbilled** blocks only (snapshot prices), fnb from cart lines, tournament lines placeholder (wired in B12), `pointsRedeemable = min(points, total)`, total. Pure computation service with exhaustive unit tests — this is the FE's bill panel contract.
**Prompt:**
```
Read backend/ARCHITECTURE.md §5.4 and docs/api-contract.md (Billing & payments).
Implement TASK B09 from TASKLIST.md: bill computation endpoint.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND unit tests cover: unbilled-blocks-only after a mid-session settle, fnb math, points cap, empty bill.
Show me the build output before saying you're finished.
```

### B10 — Payments settle + void 🔴
`branch: task/B10-payments-settle`
**Description:** The heart. `POST /payments` in **one DB TX**: validate splits sum (409 `SPLIT_MISMATCH`), wallet floor (409 `WALLET_INSUFFICIENT`), bKash/Nagad manual TrxID required (409 `PAYMENT_REF_REQUIRED`), mark blocks paid (session continues), decrement stock + movements, points earn `floor(due/20)` / redeem as capped discount, wallet spend, immutable transaction snapshot (`public_id` like `GD-2608-047`, gaming/fnb/tournament amounts), create print job **in the same TX** (render stubbed until B15 — store placeholder bytes behind the same interface), return `{transactionId, printJobId}`. `POST /payments/{id}/void` (Manager+, same-shift, full reversal: stock, ledgers, points). Tournament entries join in B12.
**Prompt:**
```
Read backend/ARCHITECTURE.md §5.2–5.4 and docs/api-contract.md (Billing & payments).
...
**Description:** `POST /shifts` (`{openingFloat}`, 409 `SHIFT_ALREADY_OPEN` per terminal), `GET /shifts/current/x-report` (takings method × category incl. tournament line), `POST /shifts/current/close` (`{countedCash}` → expected/discrepancy computed, Z print job, logout semantics; discrepancy ≠ 0 writes an alert row), expenses CRUD (`?voucher=true` → P4 job), all print renders via the B10 placeholder until B15.
**Prompt:**
```
...
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND Testcontainers covers: expected-cash math from mixed-method transactions + expenses, discrepancy alert row, second open shift 409, close produces Z job.
Show me the build output before saying you're finished.
```
...
### B15 — ESC/POS rendering P1–P5 🔴
`branch: task/B15-escpos-render`
**Description:** escpos-coffee renderer producing **bytes + 48-col text** at job creation, stored in `print_jobs.rendered`/`rendered_text`, replacing the B10 placeholder. Templates per `docs/design.md` §5: P1 sale ticket (incl. tender rows, loyalty line, Code 128 `GS k`), P2/P3 Z/X, P4 expense voucher, P5 tournament stub (inverted band, TOKEN #NN double-height, QR `GS ( k` with `qr_token`, appended to P1 in the same job). Reprint band. 80mm default, 58mm config switch. **Golden-file tests** lock bytes + text.
**Prompt:**
```
Read docs/design.md §5 (all templates + barcode/QR specs) and backend/ARCHITECTURE.md §5.5.
Implement TASK B15 from TASKLIST.md: ESC/POS render for P1–P5 + golden files.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND golden-file tests exist per template (bytes + rendered_text) incl. P5 token/QR and the reprint band; settle now stores real renders.
Show me the build output before saying you're finished.
```

### B16 — Print queue, USB device, retry/reprint
`branch: task/B16-print-queue`
...
Implement TASK B16 from TASKLIST.md: print queue worker + device port + retry/reprint endpoints.
Follow the GLOBAL RULES in TASKLIST.md.
...
### B17 — SSE hub + alerts
`branch: task/B17-sse-alerts`
**Description:** `GET /events` SSE with `station-update` (sessions AND match timers), `tournament-update`, `alert`, `printer-status`, `sync-status`, emitted post-commit from session/tournament/printing/shift services; payloads identical to GET shapes. Alerts table + feed endpoints (low-stock, discrepancy, printer) with read flags.
**Prompt:**
```
Read backend/ARCHITECTURE.md §4.5 and docs/api-contract.md (Live updates & sync).
Implement TASK B17 from TASKLIST.md: SSE hub + alert feed.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND an integration test subscribes to /events and receives station-update on block change and tournament-update on extend; alert rows created on discrepancy + printer failure.
Show me the build output before saying you're finished.
```

### B18 — Reports + Overview aggregates
`branch: task/B18-reports-overview`
**Description:** Endpoints backing S2/S9 per `docs/design.md`: KPIs (occupancy, revenue, avg ticket, net profit = takings − expenses), 30-day + day-of-week trends, 14-day stacked trend, per-station utilisation, busiest hours, top sellers, stock watchlist, staff shift-close history. Manager+ guard (Overview admin-only). Read-only SQL aggregates — no stored rollups.
**Prompt:**
```
Read docs/design.md (S2, S9) and backend/ARCHITECTURE.md §4.3, §4.6.
Implement TASK B18 from TASKLIST.md: report + overview endpoints.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: mvn verify green AND Testcontainers seeds transactions/expenses across days and asserts each aggregate incl. the tournament revenue split; cashier 403.
Show me the build output before saying you're finished.
```

### B19 — Terminal settings + login background
`branch: task/B19-terminal-settings`
...
Implement TASK B19 from TASKLIST.md: terminal settings + login-bg upload.
Follow the GLOBAL RULES in TASKLIST.md.
...
### B20 — Sync outbox + e2e money-path test 🔴
`branch: task/B20-sync-e2e`
**Description:** Transactional outbox rows written inside every money/inventory/tournament mutation TX (retrofit check across B06–B16); 30 s pusher batching to cloud `POST /sync/push` (idempotent by op id, `SYNC_TOKEN` auth); `GET /sync/status`. Dev-profile demo seed script. Then the **release-gate e2e** (Testcontainers): open shift → session → blocks → fnb → tournament entry sale → auto-bracket at cap → start match → winner → champion → split settle → print jobs → shift close with tournament line + discrepancy → outbox drained to a fake cloud.
**Prompt:**
```
Read backend/ARCHITECTURE.md §5.8 and docs/backend-architecture.md §8–§10.
Implement TASK B20 from TASKLIST.md: sync outbox + pusher + the full e2e money-path test.
Follow the GLOBAL RULES in TASKLIST.md.
...
**Description:** Hand-rolled `components/ui`: Button (5 variants × states incl. loading, centered labels), Tag, Dialog, FieldInput, SegmentedChoice, ChipSelect, DataTable, StatTile, BarChart (plain SVG), ProgressBar, AvatarSwatch, ImagePicker — variants/states/props exactly per `docs/design.md` §2, cva-driven, focus-visible outlines, 45% disabled opacity.
**Prompt:**
```
...
**Description:** `lib/api.ts` (auth header, per-intent Idempotency-Key, error-envelope → typed errors, 401 → one silent refresh → logout), OpenAPI type-gen from the backend (`/v3/api-docs`) wired into CI (drift fails build), TanStack Query provider + canonical query keys (`frontend/ARCHITECTURE.md` §4.1), Zod schemas in `features/*/schemas.ts`, `lib/time.ts` (server-offset clock for all countdowns), `lib/money.ts` (integer BDT format).
**Prompt:**
```
...
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover NAV[role] visibility for all 3 roles, lockout state rendering, middleware redirect, auto-lock trigger.
Show me the build output before saying you're finished.
```

### F05 — SSE + sync status
`branch: task/F05-sse-sync`
**Description:** `lib/sse.ts` subscribing to `/events` and writing into the canonical query keys (`station-update`, `tournament-update`, `alert`, `printer-status`, `sync-status`); 10 s polling fallback + `refetchOnReconnect`; `features/sync/use-sync-status.ts` driving the SyncChip (synced / syncing / offline since HH:MM).
**Prompt:**
```
...
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests prove an SSE station-update mutates the query cache, fallback polling activates when SSE drops, chip states render.
Show me the build output before saying you're finished.
```

## PHASE F1 — Floor & POS (Week 3–4)

### F06 — Floor (S3)
`branch: task/F06-floor`
**Description:** StationCard (all 7 variants incl. reserved/maintenance), CountdownClock (panel/card sizes, running/paused/overtime), SessionPanel (start no-clock, ±30-min blocks with optimistic update + `BLOCKS_CONSUMED` rollback, start/pause/resume, bill link, end blocked on balance with `SESSION_HAS_BALANCE` notice), tournament-match display on reserved stations (match countdown, players, "match over"), empty/error/permission states per design.md §1.
**Prompt:**
```
Read docs/design.md (S3, StationCard/SessionPanel/CountdownClock rows) and docs/tournaments.md §4 (floor integration).
Implement TASK F06 from TASKLIST.md: Floor screen.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover countdown from remainingSeconds+offset, optimistic block rollback on 409, reserved-station render, end-blocked state.
Show me the build output before saying you're finished.
```

### F07 — POS (S4): menu, cart, member, tournament entries 🔴
`branch: task/F07-pos`
**Description:** Menu grid by category incl. **Tournament** category (fee, slots left, full=disabled), station/counter toggle, BillPanel + CartLine (optimistic, qty-min removes), MemberSearch (attach, auto-fill, no-match notice), RedeemStepper (None/100/200/Max capped), player-name field when an entry is in cart (member auto-fill / free text / "Walk-in guest"), 1024–1279 ticket-column collapse. Settle itself lands in F08.
**Prompt:**
```
Read docs/design.md (S4 + component rows) and docs/tournaments.md §5.
Implement TASK F07 from TASKLIST.md: POS screen (menu, cart, member, tournament category).
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover cart optimistic add/remove with server reconciliation, redeem cap math, tournament card disabled when full, player-name field logic.
Show me the build output before saying you're finished.
```

### F08 — Payments + ticket preview
`branch: task/F08-payments`
**Description:** PaymentSplit (cash/bkash/nagad/wallet amounts, TrxID field required for MFS), settle via single `POST /payments` — **never optimistic**, failure keeps the bill intact with retry (409 code → message per state table), success → 80mm ticket preview from `GET /print-jobs/{id}/render` + job status polling (`use-print-job.ts`), `entryTokens` display for tournament stubs, member points earned line.
**Prompt:**
```
Read docs/design.md (S4 settle + P1/P5) and docs/api-contract.md (Billing & payments).
Implement TASK F08 from TASKLIST.md: split payment + settle + ticket preview.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover split-sum validation, SPLIT_MISMATCH keeps bill intact, PAYMENT_REF_REQUIRED gating, settle → preview render fetch, no optimistic write anywhere in the path.
Show me the build output before saying you're finished.
```

### F09 — Members (S6 + S6a)
`branch: task/F09-members`
**Description:** Member search/table, detail (wallet, points, visits), top-up + redeem-to-wallet dialogs (idempotent mutations), NewMemberDialog: register, optional opening top-up, "Save & seat on «station»" → starts a session and routes to Floor.
**Prompt:**
```
...
## PHASE F2 — Tournaments & operations (Week 4)

### F10 — Tournaments (S12) 🔴
`branch: task/F10-tournaments`
...
Implement TASK F10 from TASKLIST.md: tournaments screen incl. bracket, match board, role rails.
Follow the GLOBAL RULES in TASKLIST.md.
...
### F11 — Shift close (S7) + Expenses (S8)
`branch: task/F11-shift-expenses`
**Description:** S7: X-report by method, tournament-entries reconciliation strip, editable drawer count with **live discrepancy**, petty-cash list, Z print → closes shift → login. S8: expense table + record form (category chips), voucher print option.
**Prompt:**
```
Read docs/design.md (S7, S8).
Implement TASK F11 from TASKLIST.md: shift close + expenses screens.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover live discrepancy math, close flow → logout redirect, expense form validation + voucher job trigger; shift close never optimistic.
Show me the build output before saying you're finished.
```

### F12 — Inventory (S5) + Setup (S10)
`branch: task/F12-inventory-setup`
**Description:** S5 read-only stock table + low-stock rail + equipment register. S10 role-sectioned: Admin sees stations/pricing/staff (add with PIN, remove → `STAFF_ON_SHIFT` notice) + menu; Manager sees menu & stock only; printing card (devices, default, 80/58 toggle, test ticket).
**Prompt:**
```
Read docs/design.md (S5, S10) and frontend/ARCHITECTURE.md §4.3.
Implement TASK F12 from TASKLIST.md: inventory + setup screens.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover role-sectioning (manager vs admin vs cashier-hidden), staff-on-shift 409 notice, printer default persistence.
Show me the build output before saying you're finished.
```

### F13 — Overview (S2) + Reports (S9)
`branch: task/F13-overview-reports`
**Description:** S2: stat tiles, horizontally scrolling live-station cards (click → Floor), 30-day + day-of-week trends (plain SVG bars), stock watchlist, staff & shift closes, collapsible AlertsRail (bell + unread badge); stale-data banner with last-sync. S9: KPIs, 14-day stacked trend, per-station utilisation, busiest hours, top sellers; "Not enough data yet" per chart.
**Prompt:**
```
Read docs/design.md (S2, S9).
Implement TASK F13 from TASKLIST.md: overview + reports screens.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND tests cover admin-only redirect for S2, empty-chart states, alerts rail unread badge from the alerts query.
Show me the build output before saying you're finished.
```

### F14 — Settings (S13) + Print preview (S11)
`branch: task/F14-settings-print`
**Description:** S13 per design.md §6: theme/text-size/accent applied **instantly** from local state + persisted via `PUT /terminal-settings`; inline pre-paint script sets `data-theme` (no flash); login-bg ImagePicker; sound, auto-lock, receipt copies; avatar color via `/me/prefs`. S11: exact character-grid render from the server, job states (rendering/ready/queued/failed/retry), reprint with required reason picker.
**Prompt:**
```
Read docs/design.md §6 + §5 (S11 states) and frontend/ARCHITECTURE.md §5.5–5.6.
Implement TASK F14 from TASKLIST.md: settings + print preview screens.
Follow the GLOBAL RULES in TASKLIST.md.
...
## PHASE F3 — Hardening (Week 4–5)

### F15 — State-coverage + responsive pass
`branch: task/F15-state-coverage`
**Description:** Sweep every screen against the design.md §1 state table (default/loading skeleton matching layout/empty/error/permission-denied) and §4 breakpoints (1280 collapse, 768 icon sidebar + drawer, <768 notice). Fix gaps; add the state-table assertion tests the architecture demands. No new features.
**Prompt:**
```
Read docs/design.md §1 (state coverage table) + §4 and frontend/ARCHITECTURE.md §5.10.
Implement TASK F15 from TASKLIST.md: state-coverage + responsive audit across all screens.
Follow the GLOBAL RULES in TASKLIST.md.
Done when: npm run build && npm test green AND every screen has state-table tests passing; list each gap you fixed.
Show me the build output before saying you're finished.
```

### F16 — Playwright e2e suite 🔴
`branch: task/F16-playwright-e2e`
**Description:** Playwright against the seeded dev backend (B20 seed): session → blocks → bill → split settle → ticket · counter sale · member redeem · tournament entry sale → auto-bracket at cap → start match → winner → champion · shift close with discrepancy incl. tournament line · print retry after simulated printer-offline. Wire OpenAPI type-drift check + both test suites into CI. **Release gate.**
**Prompt:**
```
Read frontend/ARCHITECTURE.md §6 and TASKLIST.md B20 (seeded backend).
Implement TASK F16 from TASKLIST.md: Playwright money-path suite + CI wiring.
Follow the GLOBAL RULES in TASKLIST.md.
...
## Schedule (1 engineer, ~4–5 weeks)

| Week | Tasks | Theme |
|---|---|---|
| 1 | B01–B06 | Backend foundation + auth + sessions |
| 2 | B07–B12 | Catalog, members, **money path**, tournament core |
| 3 | B13–B17 · F01–F03 | Bracket, matches, printing, SSE · FE foundation starts |
| 4 | B18–B20 · F04–F11 | Backend wrap-up · shell, floor, POS, payments, tournaments UI |
| 5 | F12–F16 | Remaining screens, settings, hardening, e2e |

🔴 heavy tasks (B10, B12, B15, B20, F07, F10, F16): budget 1.5–2 sessions; the rest ~1.

## Dependency notes
- B01–B04 block all backend tasks; B10 (settle) blocks B11/B12; B12 blocks B13/B14; B15 blocks B16's real bytes (B16's queue can use the fake port).
- F01–F05 block all screens; F03's type-gen needs the backend running (any state ≥ B04 — regenerate as endpoints land).
- F07/F08 need B10; F10 needs B13/B14; F14 needs B19; F16 needs **everything** + B20's seed.
- Print render placeholder (B10) is intentional — do not build P1–P5 early; B15 swaps it behind the same interface.
- Cut line if the month runs tight (from `docs/backend-architecture.md` §11): report charts (keep summary numbers) → alerts feed (keep discrepancy alert) → cloud sync (nightly dump only) → tournament finance panel. Cannot cut: money path, idempotency, printing, bracket integrity, shift close.
